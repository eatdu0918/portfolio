...
> `resume-ko-sanitized.md`와 동일한 도메인 일반화. **요약·bullet 균형**을 프론트–백엔드–인프라 End-to-end에 맞췄습니다.

---

## 핵심 요약 (풀스택 균형)

Vue 3·React/Next.js **프론트엔드**와 Spring Boot 3 **백엔드**, **PostgreSQL·Redis·MongoDB·MQ**를 아우르는 데이터·메시징 레이어, **Docker·Kubernetes·CI/CD**까지 포함해 제품을 끝에서 끝까지 설계·구현한 풀스택 개발자입니다. 지도·상황 시각화, 실시간 WebSocket·Pub/Sub, **RabbitMQ 기반 AI 추론 파이프라인**, Electron·멀티 클라이언트 배포 경험이 있습니다. 개인 프로젝트로 Next.js 풀스택 블로그와 Spring Cloud Gateway·Kafka·K8s **MSA 레퍼런스**를 유지합니다.
...
- **Data:** MariaDB·MongoDB 듀얼 DB, 비동기 큐(RabbitMQ), Python 작업자.
...
한 줄로 요약 시: **Vue/React/Next · Spring Boot · PostgreSQL/MySQL · Redis/RabbitMQ/Kafka · Docker/K8s** 순으로 나열해 풀스택 인상을 줍니다.

---

*문장형 전체 경력은 `resume-ko-sanitized.md`를 복사한 뒤, 위 패턴으로 bullet을 재정렬·촘촘히 맞추면 됩니다.*

...
하나의 제품 안에서 **3D 상황 시각화**, **현장(Edge) 데이터 유입**, **관리(Admin) 마스터 정합성**이 동시에 돌아가야 한다. 한 코드베이스·한 데이터베이스에 몰면 배포·권한·성능이 한꺼번에 엉키기 쉬워, **역할별 Vue 3 클라이언트 3종 + Spring Boot 3 멀티모듈 API + 공유 라이브러리**를 전제로 구조부터 잡았다.

## 내가 풀려던 문제

1. AI 분석은 **비동기·역호출**인데 상황판은 **실시간**이어야 한다 — HTTP 동기 호출로는 시간 스케일이 맞지 않는다.
2. 지휘용·관리용·현장용 UI는 **같은 도메인 모델**을 쓰지만 **노출 API와 권한·배포 경계**가 달라야 한다.
3. 현장 단말은 네트워크가 단절돼도 동작해야 하므로 **중앙 PostgreSQL 전체를 엣지로 내려줄 수 없다.**

## 접근과 결과 (요약)

- AI 연동은 **세션 키 기반 콜백 + Redis·WebSocket STOMP 실시간 푸시**로 API 스레드를 붙잡지 않고 결과만 지휘 화면에 반영했다.
- **Spring Boot 3 멀티모듈(main-api · admin-api · common)** 로 배포·권한 경계를 나누고, **MapStruct + MyBatis**로 DTO/VO 계약을 컴파일 타임에 고정했다.
- 엣지는 **필요한 테이블만 SQLite로 동기화**해 용량·유출 범위를 줄이고 오프라인 연속성을 확보했다.
- 프런트는 **Vue 3 + Pinia + Cesium 1.128**을 공통 기반으로, 지휘(PrimeVue)·관리(PrimeVue)·엣지(Quasar) 세 클라이언트를 같은 도메인 타입·API 스키마 위에서 병렬 개발했다.

## 제품 관점에서의 가치

지휘관은 **한 화면에서 3D 상황·AI 결과·실시간 장비 상태**를 같이 본다. 현장은 **HLS 스트리밍·보고 API**로 끊김 없이 데이터를 올리고, 관리자는 **별도 Admin**에서 마스터 데이터를 깨끗이 유지한다 — 운영 책임이 코드 구조와 맞닿게 정렬된다.

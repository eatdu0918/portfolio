...
[이 프로젝트](https://aetem-v2-cop.dev-aistudio.com/)는 **규격 렌더링**과 **운용 속도(다중 맵·대량 심볼·Draw)** 를 동시에 만족해야 하는 Vue 3 SPA다. v1에서 겪었던 화면-SDK 결합도를 끊어내기 위해, **Vue 컴포넌트가 OpenLayers·군사 렌더러 세부를 직접 들여다보지 않게** TypeScript 기반 퍼사드로 경계를 먼저 잡았다.
...
1. **지도·심볼·Draw·ORBAT**가 같은 컴포넌트에 얽히면 변경 비용이 기하급수로 늘어난다.
2. **3개의 독립 맵**은 화면마다 레이어·가시성·편집 권한이 달라야 하는데, 전역 단일 Pinia 스토어로는 간섭이 난다.
3. 심볼은 **대량·반복 렌더링**, 그래픽은 **미리보기·GeoJSON 반영** 이 중요해 **같은 렌더 경로로 묶기 어렵다.**
4. SDK·규격이 겹치면 **코드만으로는 의도가 전달되지 않아**, 신규 투입자와 QA가 같은 그림을 갖기 어렵다.

## 접근과 결과 (요약)

- **Provider(Facade) + 내부 Manager/Renderer** 로 외부 의존을 한 축에 모아, Screen 1~3을 같은 패턴으로 확장했다.
- **뷰·경로 단위 키로 샤딩한 Pinia 레이어 상태**와 assignment / rule 모드로 "한 제품, 화면마다 다른 뷰"를 중복 없이 표현했다.
- **이중 렌더러 + LRU SVG 캐시(120)** 로 심볼 반복 비용을 줄이고, 그래픽은 **Draw → 미리보기 → GeoJSON 확정** 파이프라인만 퍼사드로 공개했다.
- **TypeScript 타입**으로 퍼사드 진입점·상태 샤딩 키·그래픽 단계를 컴파일 타임에 고정하고, 저장소 내 기술 문서(모듈 README·ADR·시퀀스 다이어그램·온보딩 체크리스트)로 공개 계약과 내부 구현 경계를 글로 남겼다.

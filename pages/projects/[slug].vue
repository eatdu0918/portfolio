...
<template>
  <div class="pt-24">
    <!-- Project Hero -->
    <section class="py-10 sm:py-14 bg-white border-b border-surface-300">
      <div class="section-container">
        <NuxtLink
          to="/projects"
          class="inline-flex items-center gap-1 text-sm text-surface-500 hover:text-black transition-colors mb-5"
        >
        ...
        <div v-if="project" class="w-full max-w-4xl">
          <div class="flex flex-wrap items-center gap-2 mb-3">
            <span
              v-if="project.workType"
              class="text-[10px] font-bold px-2.5 py-1 rounded-[999px]"
              :class="project.workType === 'company' ? 'bg-black text-white' : 'bg-brand-300 text-black'"
            >
              {{ project.workType === 'company' ? '실무' : '개인' }}
            </span>
          </div>

          <h1 class="text-2xl sm:text-3xl font-display font-bold text-black tracking-tight mb-2">
            {{ project.title }}
          </h1>

          <div class="flex flex-wrap items-center gap-3 text-sm text-surface-500 mb-4 font-mono">
            <span v-if="project.period">{{ project.period }}</span>
            <span v-if="project.role" class="text-surface-400">{{ project.role }}</span>
          </div>

          <p class="text-sm text-surface-600 leading-relaxed max-w-2xl">
            {{ project.summary }}
          </p>
          ...
          <div class="flex flex-wrap items-center gap-4 mt-5">
            ...
            <div v-if="project.demoLinks?.length" class="flex flex-wrap gap-2">
              <a
                ...
                class="inline-flex items-center gap-1.5 px-4 py-2 bg-black text-white rounded-[999px] text-xs font-semibold hover:bg-surface-800 transition-colors"
              >
                <Icon :name="getIconForLabel(link.label)" class="w-3.5 h-3.5" />
                {{ link.label }}
              </a>
            </div>

            <!-- Test Accounts -->
            <div v-if="project.testAccounts?.length" class="mt-4 p-4 bg-surface-100 border border-surface-300 rounded-[8px] max-w-2xl w-full">
              <h3 class="text-[10px] font-bold text-black mb-2.5 flex items-center gap-1.5 uppercase tracking-wider">
                <Icon name="heroicons:key-20-solid" class="w-3.5 h-3.5" />
                테스트 계정 안내
              </h3>
    ...
    <!-- Tabs -->
    <section class="sticky top-16 z-40 bg-white border-b border-surface-300">
      <div class="section-container">
        <div class="flex overflow-x-auto gap-1.5 py-3 -mx-4 px-4 sm:mx-0 sm:px-0 scrollbar-hide">
          <button
            v-for="tab in tabs"
            :key="tab.key"
            class="flex items-center gap-1.5 px-4 py-2 rounded-[999px] text-xs font-semibold whitespace-nowrap transition-colors"
      ...
      <div class="section-container">
        ...
        <div v-if="activeTab === 'overview'" class="w-full">
          ...
          <div v-if="project?.techStack" class="mb-10">
            <h2 class="text-lg font-bold text-black mb-4 tracking-tight">기술 스택</h2>
            <TechStackSection :tech-stack="project.techStack" />
          </div>

          <!-- Highlights -->
          <div v-if="project?.highlights?.length" class="mb-10">
            <h2 class="text-lg font-bold text-black mb-4 tracking-tight">주요 성과</h2>
